...
from .benchmarks import RecentlyPopularRecommender, ContentBasedRecommender, ItemCooccurrenceRecommender, \
    ItemKNNRecommender, SessionBasedKNNRecommender, SequentialRulesRecommender
...
tf.flags.DEFINE_integer('train_negative_samples_from_buffer', default=10,
                        help='Training Negative samples from recent clicks buffer')
tf.flags.DEFINE_integer('eval_total_negative_samples', default=20, help='Total negative samples for evaluation')
tf.flags.DEFINE_integer('eval_negative_samples_from_buffer', default=50,
                        help='Eval. Negative samples from recent clicks buffer')
...
tf.flags.DEFINE_bool('eval_metrics_by_session_position', default=False,
                     help='Computes eval metrics at each position within session (e.g. 1st click, 2nd click)')
...
tf.flags.DEFINE_string('embedding_path', default='',
                       help='Path to embeddings numpy matrix')
...
tf.flags.DEFINE_integer('training_hours_for_each_eval', default=5,
                        help='Train model for N hours before evaluation of the next hour')
tf.flags.DEFINE_integer('save_results_each_n_evals', default=1,
                        help='Saves to disk and uploads to GCS (ML Engine) the incremental evaluation results each N evaluations')
tf.flags.DEFINE_bool('save_eval_sessions_negative_samples', default=False,
                     help='Save negative samples of each session during evaluation')
tf.flags.DEFINE_bool('use_local_cache_model_dir', default=False,
                     help='Persists checkpoints and events in a local temp file, copying to GCS in the end of the process (useful for ML Engine jobs, because saving and loading checkpoints slows training job)')
...
def nar_module_model_fn(features, labels, mode, params):
    ...
    model = NARModuleModel(mode, features, labels,
                           ...
                           content_article_embeddings_shape=params['content_article_embeddings_matrix'].shape,
    ...
    if not FLAGS.disable_eval_benchmarks:
        eval_benchmark_classifiers = [{'recommender': RecentlyPopularRecommender, 'params': {}},
                                      ...
                                      {'recommender': SessionBasedKNNRecommender,
                                       'params': {'sessions_buffer_size': 3000,
                                                  # Buffer size of last processed sessions
                                                  'candidate_sessions_sample_size': 200,
                                                  # Number of candidate near sessions to sample
                                                  'sampling_strategy': 'recent',  # (recent,random)
                                                  'nearest_neighbor_session_for_scoring': 50,
                                                  # Nearest neighbors to compute item scores
                                      ...
                                      #             'content_article_embeddings_matrix': params[
                                      #                 'content_article_embeddings_matrix']}},
                                      {'recommender': SequentialRulesRecommender,
                                       'params': {'max_clicks_dist': 10,
                                                  # Max number of clicks to walk back in the session from the currently viewed item. (Default value: 10)
                                                  'dist_between_clicks_decay': 'div'
                                                  # Decay function for distance between two items clicks within a session (linear, same, div, log, qudratic). (Default value: div)
                                                  }}
                                      ]
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
        ...
        return tf.estimator.EstimatorSpec(
            mode, loss=model.total_loss, train_op=model.train,
            training_chief_hooks=hooks,
            scaffold=tf.train.Scaffold(
                init_feed_dict={
                    model.content_article_embeddings_matrix: params['content_article_embeddings_matrix']}))
    elif mode == tf.estimator.ModeKeys.EVAL:
        ...
        return tf.estimator.EstimatorSpec(
            mode, loss=model.total_loss, eval_metric_ops=eval_metrics,
...
def main(unused_argv):
    try:
        ...
        if trial is not None:
            ...
            tf.logging.info(
                "Hyperparameter Tuning - Trial {} - model_dir = {} - gcs_model_output_dir = {} ".format(trial,
                                                                                                        model_output_dir,
                                                                                                        gcs_model_output_dir))
        ...
        if FLAGS.warmup_model_dir != None:
            tf.logging.info(
                'Copying model outputs from previous job ({}) for warm start'.format(FLAGS.warmup_model_dir))
            ...
            tf.logging.info(
                'Files copied from GCS to warm start training: {}'.format(local_files_after_download_to_debug))
        ...
        content_article_embeddings_matrix = load_external_embeddings(FLAGS.embedding_path)
        ...
        eval_sessions_metrics_log = []
        clicked_items_state = ClickedItemsState(FLAGS.recent_clicks_buffer_size,
